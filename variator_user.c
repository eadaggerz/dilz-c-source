...
*/
{
	 ...
	 initial_population[i] = add_individual(ind);
	    ...
	    for(ki = 0; ki < ind->n; ki++){
		     sprintf(ind_str,"%.5f",ind->x[ki]);
		     fputs(ind_str,inipop);
		     fputs(" ",inipop);
	     }
     	fputs("\n",inipop);
     } 
...
int load_matrix(){
	...
	if(pfile){
		...
		printf("Error al aperturar el archivo.\n");
		return 0;
	}
...
int mutation(individual *ind)
{
	 ...
	 if (drand(1) <= variable_mutation_probability)
	 {
	     ...
	     double ub = current;    /* upper bound of variable i */
...
{    
    ...
    //calculando E(Rp) rendimiento esperado del individuo
    // esto es Suma de (rendimiento * gen)
	...
	/*double x = -10 + 20 * ind->x[i];
      ...
      f += ind->x[i]*performance[i];
    }


    //calculando VAR(Rp) variabilidad de la cartera p
    for(i=0; i < n; i++){
    	for(j=0; j < n; j++){
    		f2 += (ind->x[i]*ind->x[j]*cvmatrix[i][j]);
    	}
    }

    
    ind->f[0] = 1/f; // esto es porque dtlz solo minimiza, mientras mas grande sea el numero menor sera el resultado de la division por eso g/f
...
individual *new_individual()
{
     ...
     for (i = 0; i < number_decision_variables; i++)
     {
     		...
     		/*if(i+1 == number_decision_variables){

     			return_ind->x[i] = current;

     		}else{
     			return_ind->x[i] = drand(current);
     			current = current - return_ind->x[i];	
     		}*/
     		return_ind->x[i] = drand(current);
